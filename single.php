...
<?php $single_banner_img = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), 'custom-tb-size'); ?>
<div id="banner" style="background-image: url('<?php echo !empty($single_banner_img[0]) ? esc_url($single_banner_img[0]) : ''; ?>')">
...
<!--    <div id="overlay"></div>-->
    <section id="posts-container">
        <section class="post-wrapper <?php echo has_post_thumbnail() ? 'has-thumbnail' : ''; ?>">
        <?php
        the_post();
        ?>
        <?php
...
<!--                <span class="post-meta-opt post-meta-visited">--><?php //_e("访问次数","mingof")?><!--：--><?php //echo get_post_meta($post->ID,'_visited', true) ?><!--</span>-->
            ...
            <div>
				本文链接：<a href="<?php echo esc_url(get_permalink($post->ID));?>"  style="text-decoration: underline;"><?php echo esc_html(urldecode(get_permalink($post->ID)));?></a>
			</div>
        </article>
